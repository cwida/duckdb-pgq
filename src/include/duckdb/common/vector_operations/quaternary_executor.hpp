...
struct QuaternaryLambdaWrapper {
	...
	static inline RESULT_TYPE Operation(FUN fun, A_TYPE a, B_TYPE b, C_TYPE c, D_TYPE d, ValidityMask &mask, idx_t idx) {
		return fun(a, b, c, d);
	}
};
...
private:
	...
	static inline void ExecuteLoop(A_TYPE *__restrict adata, B_TYPE *__restrict bdata,
	                               C_TYPE *__restrict cdata, D_TYPE *__restrict ddata, RESULT_TYPE *__restrict result_data, idx_t count,
	                               const SelectionVector &asel, const SelectionVector &bsel,
	                               const SelectionVector &csel, const SelectionVector &dsel,
	                               ValidityMask &avalidity, ValidityMask &bvalidity, ValidityMask &cvalidity, ValidityMask &dvalidity, ValidityMask &result_validity,
	                               FUN fun) {
...
public:
	...
	static void Execute(Vector &a, Vector &b, Vector &c, Vector &result, idx_t count, FUN fun) {
		ExecuteGeneric<A_TYPE, B_TYPE, C_TYPE, RESULT_TYPE, QuaternaryLambdaWrapper, FUN>(a, b, c, result, count, fun);
	}
	...
	static void ExecuteWithNulls(Vector &a, Vector &b, Vector &c, Vector &result, idx_t count, FUN fun) {
		ExecuteGeneric<A_TYPE, B_TYPE, C_TYPE, RESULT_TYPE, QuaternaryLambdaWrapperWithNulls, FUN>(a, b, c, result, count,
		                                                                                        fun);
	}
