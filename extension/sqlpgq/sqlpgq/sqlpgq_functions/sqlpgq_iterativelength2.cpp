...
struct IterativeLength2BindData : public FunctionData {
	ClientContext &context;
	string file_name;

	IterativeLength2BindData(ClientContext &context, string &file_name) : context(context), file_name(file_name) {
	}

	unique_ptr<FunctionData> Copy() override {
		return make_unique<IterativeLength2BindData>(context, file_name);
	}
};
...
static void IterativeLength2Function(DataChunk &args, ExpressionState &state, Vector &result) {
	...
	auto &info = (IterativeLength2BindData &)*func_expr.bind_info;
...
static unique_ptr<FunctionData> IterativeLength2Bind(ClientContext &context, ScalarFunction &bound_function,
...
CreateScalarFunctionInfo SQLPGQFunctions::GetIterativeLength2Function() {
	auto fun = ScalarFunction("iterativelength2",
	                          ...
	                          LogicalType::BIGINT, IterativeLength2Function, false, IterativeLength2Bind);
